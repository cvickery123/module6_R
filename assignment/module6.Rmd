...
**Answer Q.1** Uncertainty in the context of ecological forecasting means "the range of possible alternative future conditions predicted by a model". Because we cannot know the future, we cannot accurately predict a singular future outcome with complete certainty. We, therefore, need to represent all possible outcomes using uncertainty.   
...
**Answer Q.2** Knowing the uncertainty helps natural resource managers weigh their decisions against model outcomes. Model outcomes with low uncertainty would allow managers to be more confident to use the model to inform decisions and to respond to the model predictions. Models with high uncertainty could prompt a natural resource manager to collect more data to reduce uncertainty or to respond less urgently to the model prediction. In the example of a drinking water manager receiving a toxic algal bloom forecast, a model with low uncertainty would cause them to mitigate the problem quickly and effectly, whereas a model with high uncertainty would give them pause before acting. 
...
**Answer Q.4** Increasing water temperature can stress fish populations to decrease their feeding and reproduction rates. Another result of increase water temperature could be increased likelihood in algal blooms, which could create anoxic environments over time. 
...
**Answer Q.5** There looks to be a strong relationship between air and water temperature, given that increases in air temperature correspond with increases in water temperature. 
...
**Answer Q.6** The gray lines are ensemble members, which each represent an equally plausible future outcome for air temperature in the future. There are multiple gray lines because of uncertainty in our model, including driver uncertainty and process uncertainty. 
...
**Answer Q.7** Water temperature is being predicted based on the current day's air temperature and the previous day's water temperature. These parameters are put into a multiple linear regression, which allowed us to calculate the coefficients and standard errors of the parameters. 
...
**Answer Q.8** Yesterday's water temperature is a larger driver of current water temperature than air temperature because (t-1) water temperature has a larger coefficient than air temperature does. Both parameters have similar standard errors, which are less than 2%. With a general significance value of p<5%, our standard errors are acceptable and communicate a medium level of confidence in the fitted model parameter values.  
...
**Answer Q.9** Visually and according to the r2 value of 0.9, our model fits the data well. There are only a few data points that the model does not intersect when examining the plot visually. The rmse of 0.61 is a bit high on a scale of 0 to 1, so we could improve that statistic. Our mean bias is very small (3.58 e-15), so we see little difference between the model values and observed values. 
...
The predicted water temperature on the 10th day is 16.1 degrees C. 
```{r}
starting_temperature <- 15.2 #degrees C
...
**Answer Q.11** The forecasted water temperature follows the same track as the forecasted air temperature, with the exception of the starting point and therefore slope before Sept 27. This relationship might be that both are informed by past water temperature data and that there is a relationship between air temperature and water temperature. It is important to note that the NOAA air temperature forecast includes uncertainty, while this deterministic forecast for water temperature does not. 
...
**Answer Q.12** Forecast uncertainty increases further into the future, as the ensemble members diverge further from the ensemble mean as time progresses. 
...
**Answer Q.13** The probabilistic forecast presents more alternative futures than the deterministic forecast. The probabilistic forecast, therefore, shows a wider range of possible temperatures than the deterministic forecast. The differences in these two forecasts communicates that water temperature is more variable than the deterministic forecast predicted so that the water temperature could be up to a degree warmer or colder than was initially projected. Both forecasts, however, follow the same general shape, warming until September 28 or 29, then cooling off significantly thereafter. Because of this similarity, I have confidence that the water temperature will likely follow this trajectory of warming then cooling off around that date. 
...
**Answer Q.14** We could reduce parameter uncertainty with more data that we can use to train our model. As the dataset increases in longevity, our parameter uncertainty will decrease if we look back into the past long enough to more accurately train our model. 
...
**Answer Q.15** Sensors that have been out for a long time without recalibration could lead to uncertainty in the initial condition data reading. Sensor placement could also factor into initial conditions uncertainty; for example, if the sensor is placed right below an input stream, the water temperature might be colder or warmer than the overall lake temperature. 
...
**Answer Q.16** Parameter uncertainty contributes the most to total forecast uncertainty. 
...
**Answer Q.17** Forecast uncertainty is specific to the forecast model. Different water temperature forecast models would have different quantities of uncertainty because uncertainty comes from many different sources. For example, using different parameters for this forecast would most greatly influence forecast uncertainty because parameter uncertainty is the largest source of uncertainty for this forecast. 
...
**Answer Q.19** The multiple linear regression model is predicting water temperature using yesterday's water temperature, today's air temperature, and today's wind speed. The structure of this model differs from the previously fitted model with the addition of one more parameter, which adds more parameter uncertainty. Visually, this model fits the observed data less well than the previous linear regression. 
...
for(d in 2:length(forecasted_dates)) {
  ...
  #pull wind speed values
    wind_driv<-driver_data %>%
      filter()
    
  #run model - REVISE THIS!!!!!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + coeffs[4] * temp_driv$wind_speed
    #yesterday's water temp + air temp + wind speed
  #insert values back into the forecast dataframe
